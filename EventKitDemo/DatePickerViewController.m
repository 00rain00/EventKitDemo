...
@property (nonatomic, strong)CoreDataService *cd;
   ...
   NSArray *result =  [self.cd fetchCondition:request];
    DDLogDebug(@"conditionsize: %d",result.count);
    for(Condition * condition in result){
        if([condition.myKey isEqualToString:kTime]){
            DDLogDebug(@"repeated condition key found");
            flag = YES;
            break;
        }
    }
...
- (IBAction)acceptDate:(id)sender {
    ...
    if(OBJECT_IS_EMPTY(reminderID)){
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Ooops" message:@"Could not find reminder ID" preferredStyle:UIAlertControllerStyleAlert];
        ...
        if([self saveValidation:reminderID]){
            ...
            DDLogDebug(@"saving the condition");
...
//            DDLogDebug(@"%@is Dic :%d",key, [formDic[key] isKindOfClass:[NSDictionary class]]);
            ...
            [self.cd createCondition:reminderID :kTime :myValue];
        //}
